...
#
# Purpose:
#   This script is to retrain BERT using the plant science history project corpus.
#   The install and setup info follows:
# 
# Install:
# ```bash
...
import os, json, configparser, argparse, pickle
import pandas as pd
...
def get_args():
  ...
  parser.add_argument('-c', '--config_file_name', 
                      type=str,
                      help='config file name, assume to be in the working dir',
                      default='config.ini')
...
def txt_to_split_dataset(corpus_file, test_size, rand_seed):
  ...
  d_split_file = work_dir / "d_split.pickle"
  ...
  if train_file.is_file() and test_file.is_file() and d_split_file.is_file():
    ...
    dataset_to_text(d_split["train"], "train.txt")
    dataset_to_text(d_split["test"], "test.txt")
...
def retrain_bert(train_dataset, test_dataset, tokenizer, model_dir, config):
  '''Retrain BERT
  Args:
...
if __name__== '__main__':
  ...
  # Get command line argument (just config_file_name)
  args        = get_args()
  ...
  rand_seed   = int(config_d['rand_seed'])
  corpus_file = work_dir / config_d['corpus_file_name']
  model_dir   = work_dir / config_d['model_dir_name']
  test_size   = float(config_d['test_size'])
  ...
  print("Retrain model")
  retrain_bert(train_dataset, test_dataset, btz_tokenizer, model_dir, config)
